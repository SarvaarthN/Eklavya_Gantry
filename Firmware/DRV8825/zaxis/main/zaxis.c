...
#define LIM_SWITCH_PIN  GPIO_NUM_15 //to be changed

#define SCREW_PIN GPIO_NUM_4
...
void setup_stepper_pins() {
    ...
    gpio_config_t io_conf_out = {
        ...
        .pin_bit_mask = (1ULL << STEP_PIN) | (1ULL << DIR_PIN) | 
                        (1ULL << M0_PIN) | (1ULL << M1_PIN) | (1ULL << M2_PIN)| (1ULL << SCREW_PIN),
        .pull_down_en = 0,
        .pull_up_en = 0
    };
...
void single_step1(int no_steps) {
    for(int i=0; i<no_steps; i++){
        gpio_set_level(STEP_PIN, 1);
        esp_rom_delay_us(STEP_HIGH_TIME_uS);
        gpio_set_level(STEP_PIN, 0);
        esp_rom_delay_us(250 - STEP_HIGH_TIME_uS);
    }
}

void home() {
    gpio_set_level(DIR_PIN, 0); // Move towards switch
    while (gpio_get_level(LIM_SWITCH_PIN) == 0) {
...
void app_main() {
    ...
    while (1) {
        ...
        if (final_position>60){
            printf("\neeee bhaii itna nhi jaeega mai.");
        } else {
            if (current_position<final_position){
                int n=(final_position-current_position)*800;
                gpio_set_level(DIR_PIN, 1);
                gpio_set_level(SCREW_PIN, 1);
                single_step1(n);
            } else {
                int n=(current_position-final_position)*800;
                gpio_set_level(DIR_PIN, 0);
                gpio_set_level(SCREW_PIN, 0);
                single_step1(n);
            }
